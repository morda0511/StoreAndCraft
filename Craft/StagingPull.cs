            ...
            try
            {
                foreach (Piece.Requirement req in requirements)
                {
                    ...
                    foreach (Container chest in NearbyIndex.Current)
                    {
                        ...
                        {
                            // One remote consume for the remaining deficit (no Grant into backpack).
                            TransferService.Consume(chest, shared, deficit, leaveOne, itemQuality);
                            deficit = 0;
                            break;
                        }

                        int took = TransferService.Consume(chest, shared, deficit, leaveOne, itemQuality);
                        deficit -= took;
                    }
                }
            }
